...
//-----------------------------圆 trajectory----------------------------------
        ...
        // 生成环形轨迹
        quadrotor_common::Trajectory go_to_pose_traj =
            trajectory_generation_helper::polynomials::generateMinimumSnapRingTrajectory(
                segment_times, trajectory_settings, kGoToPoseTrajectorySamplingFrequency_);
        if (go_to_pose_traj.points.size() < 2) {
            return;
        }

        auto it = go_to_pose_traj.points.begin();
        auto next_it = std::next(it);

        while (next_it != go_to_pose_traj.points.end()) {
            Eigen::Vector3d dir = (next_it->position - it->position).normalized();
            it->heading = atan2(dir.y(), dir.x());  // 计算航向角
        ...
        // 处理最后一个点的 heading
        if (it != go_to_pose_traj.points.begin()) {
...
void AutoPilot<Tcontroller, Tparams>::startCallback(
  ...
  std::lock_guard<std::mutex> main_lock(main_mutex_);
  ROS_INFO_THROTTLE(0.5, "[%s] START command received", pnh_.getNamespace().c_str());

  if (autopilot_state_ == States::OFF) {
    if (state_estimate_available_) {
      if (received_state_est_.coordinate_frame == quadrotor_common::QuadStateEstimate::CoordinateFrame::WORLD 
          ||received_state_est_.coordinate_frame == quadrotor_common::QuadStateEstimate::CoordinateFrame::OPTITRACK) {
          ROS_INFO("[%s] Absolute state estimate available, taking off based on it", pnh_.getNamespace().c_str());
          setAutoPilotState(States::START);
      } else if (received_state_est_.coordinate_frame == quadrotor_common::QuadStateEstimate::CoordinateFrame::VISION 
          || received_state_est_.coordinate_frame == quadrotor_common::QuadStateEstimate::CoordinateFrame::LOCAL) {
          ROS_INFO("[%s] Relative state estimate available, switch to hover",pnh_.getNamespace().c_str());
          force_breaking_ = true;  // Ensure reference state is reset
          setAutoPilotState(States::HOVER);
      }
    } else {
          ROS_ERROR("[%s] No state estimate available, will not start", pnh_.getNamespace().c_str());
    }
  } else {
...
template <typename Tcontroller, typename Tparams>
quadrotor_common::ControlCommand AutoPilot<Tcontroller, Tparams>::start(const quadrotor_common::QuadStateEstimate& state_estimate) {
  ...
  if (timeInCurrentState() > optitrack_start_land_timeout_ 
    ...
    if (timeInCurrentState() < start_idle_duration_) {
      ...
      reference_state_.position.z() = initial_start_position_.z() + start_land_velocity_ * (timeInCurrentState() - start_idle_duration_);
      reference_state_.velocity.z() = start_land_velocity_;
      if (timeInCurrentState() < start_idle_duration_ + start_land_velocity_ / start_land_acceleration_) {
        reference_state_.acceleration.z() = start_land_acceleration_;
        reference_state_.velocity.z() = start_land_acceleration_ *(timeInCurrentState() - start_idle_duration_);
      } else {
