...
/*
  rotors_desired_motor_speed_pub_：发布器，用于向 UAV 的执行器系统发送所需的电机速度命令。
  rpg_control_command_sub_：订阅器，用于接收控制命令（例如期望的姿态或机体速率）。
  rotors_odometry_sub_：订阅器，用于接收 UAV 的里程计数据，包括位置和速度。
  motor_speed_sub_：订阅器，用于接收当前的电机速度。
  arm_interface_sub_：订阅器，用于接收电机系统的武装命令（是否允许控制）。
...
*/
  ...
  //控制系统的核心，它以固定频率运行，并根据最新的控制命令和传感器数据来控制 UAV。
...
void RPGRotorsInterface::lowLevelControlLoop(const ros::TimerEvent& time) {
  /*
    ATTITUDE：控制 UAV 的姿态（方向），然后将其转换为机体速率。
    BODY_RATES：直接控制 UAV 的机体速率（角速度）。
    ANGULAR_ACCELERATIONS：控制角加速度，并转换为电机速度。
    ROTOR_THRUSTS：直接提供每个转子的推力值，并计算相应的电机速度。
  */
  mav_msgs::Actuators desired_motor_speed; //电机速度
...
//通过四元数误差表示法和比例控制增益，将期望的姿态转换为机体速率。
...
//使用混合器模型，将计算出的力矩和推力转换为各个电机的速度命令。
