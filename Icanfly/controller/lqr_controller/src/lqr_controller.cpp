...
// 新增：计算连续动力学导数
Eigen::VectorXd LQRController::computeStateDot(const Eigen::VectorXd &x, const Eigen::VectorXd &u)
{
  Eigen::VectorXd x_dot(state_dim);
  // 状态分解
  double p_x = x(0), p_y = x(1), p_z = x(2);
...
//     // ROS_WARN("State derivative norm: %f", x_dot.norm());
...
{
  // 1. 计算 k1
  Eigen::VectorXd k1 = computeStateDot(x, u);
  // 2. 计算 k2：在 x + dt/2*k1 处求导
  Eigen::VectorXd k2 = computeStateDot(x + 0.5 * dt * k1, u);
  // 3. 计算 k3：在 x + dt/2*k2 处求导
  Eigen::VectorXd k3 = computeStateDot(x + 0.5 * dt * k2, u);
  // 4. 计算 k4：在 x + dt*k3 处求导
  Eigen::VectorXd k4 = computeStateDot(x + dt * k3, u);

  // RK4 综合
  Eigen::VectorXd x_next = x + dt/6.0 * (k1 + 2*k2 + 2*k3 + k4);

  // 对四元数部分归一化（防止数值漂移），四元数位于下标3~6
  Eigen::Vector4d q_next = x_next.segment(3, 4);
  q_next.normalize();
  x_next.segment(3, 4) = q_next;
          ...
          std::vector<Eigen::VectorXd> &x_seq_out, std::vector<Eigen::VectorXd> &u_seq_out)
{
    ...
    for (int iter = 0; iter < max_iter; iter++) {
        ...
        for (int t = horizon - 1; t >= 0; t--) {
            ...
            ROS_ERROR("Q_uu is not positive definite, adding regularization!");
            Q_uu += 1e-6 * Eigen::MatrixXd::Identity(Q_uu.rows(), Q_uu.cols());
            }
  ...
  nh_private_("~")
{
  ...
  Qf = MatrixXd::Zero(state_dim, state_dim);
  // 根据设计要求，例如对位置赋较高权重：Q = diag(100, 100, 100, 10, 10, 10, 10, 1, 1, 1)
  // Qf(0, 0) = 100;
  // Qf(1, 1) = 100;
  // Qf(2, 2) = 100;
  // Qf(3, 3) = 10;  // q_w（虽在四元数中但可适当设置）
  // Qf(4, 4) = 10;
  // Qf(5, 5) = 10;
  // Qf(6, 6) = 10;
  // Qf(7, 7) = 10;
  ...
  R_ = MatrixXd::Zero(control_dim, control_dim);
  R_(0, 0) = 0.1;  // 对推力的惩罚较小
  R_(1, 1) =  0.2;  // 对角速率1的惩罚
  R_(2, 2) =  0.1;  // 对角速率2的惩罚
  R_(3, 3) =  0.1;  // 对角速率3的惩罚
...
std::vector<Eigen::VectorXd> LQRController::computeReferenceThrust(
    const quadrotor_common::Trajectory& ref) {

  
  std::vector<Eigen::VectorXd> u_ref_seq;


  // 打印轨迹点数量
  ROS_INFO("Number of trajectory points: %zu", ref.points.size());
   
  // 如果只有一个轨迹点，则将该点控制输入填满整个预测时域（horizon）
  if (ref.points.size() == 1) {
    const auto &point = ref.points.front();
    Eigen::VectorXd u_ref(control_dim);  // 假设 control_dim == 4

    // 根据参考加速度和重力补偿计算推力
    double des_acc_z = point.acceleration.z() + 9.81;
    
    // 构造四元数并归一化
    Eigen::Quaterniond q(point.orientation.w(), point.orientation.x(),
                         point.orientation.y(), point.orientation.z());
    q.normalize();
    
    // 计算推力系数
    double thrust_coeff = q.w() * q.w() - q.x() * q.x() - q.y() * q.y() + q.z() * q.z();
    
    // 得到参考推力
    u_ref(0) = des_acc_z / thrust_coeff;
    
    // 直接使用轨迹点中的角速度作为参考角速率
    u_ref.segment(1, 3) = point.bodyrates;
    
    // 重复填充 horizon 次
    for (int i = 0; i < horizon; i++) {
      u_ref_seq.push_back(u_ref);
    }
    return u_ref_seq;
  }
  ...
  for (const auto& point : ref.points) {
    ...
    // 计算推力系数：注意这里的系数计算方式应与动力学模型匹配
    double thrust_coeff = q.w() * q.w() - q.x() * q.x() - q.y() * q.y() + q.z() * q.z();
    
    // 参考推力
    u_ref(0) = des_acc_z / thrust_coeff;
    // u_ref(0) = des_acc_z ;
    // 直接使用轨迹点中的角速度数据作为参考角速率
    u_ref.segment(1, 3) = point.bodyrates;
...
std::vector<Eigen::VectorXd> LQRController::constructReferenceVector(
  const quadrotor_common::Trajectory& ref) 
{
  std::vector<Eigen::VectorXd> ref_states;
  ...
  if (ref.points.size() == 1) {
    ...
    x_ref(0) = point.position.x();
    x_ref(1) = point.position.y();
    x_ref(2) = point.position.z();
    
    x_ref(3) = point.orientation.w();
    x_ref(4) = point.orientation.x();
    x_ref(5) = point.orientation.y();
    x_ref(6) = point.orientation.z();
    
    x_ref(7) = point.velocity.x();
    x_ref(8) = point.velocity.y();
    x_ref(9) = point.velocity.z();
    
    
    // 重复填充 horizon 次
    for (int i = 0; i < horizon+1; i++) {
      ref_states.push_back(x_ref);
    }
    return ref_states;
  }
...
quadrotor_common::ControlCommand LQRController::run(
  ...
  // const auto& reference_state = reference_trajectory.points.front();
  computeControl(state_estimate, reference_trajectory, &command);
  return command;
}
...
quadrotor_common::ControlCommand* command)
{
  ...
  std::vector<Eigen::VectorXd> x_ref_traj = constructReferenceVector(reference_trajectory);

  // 打印当前状态的位置信息
  ROS_INFO("Current State Position: x = %.3f, y = %.3f, z = %.3f", 
           x0(0), x0(1), x0(2));
  
  // 确保参考轨迹不为空
  if (!x_ref_traj.empty()) {
      Eigen::VectorXd x_ref_first = x_ref_traj[0];
  
      // 打印参考轨迹的第一个点的位置信息
      ROS_INFO("Reference Trajectory First Point Position: x = %.3f, y = %.3f, z = %.3f", 
               x_ref_first(0), x_ref_first(1), x_ref_first(2));
  } else {
      ROS_WARN("Reference trajectory is empty! Check trajectory generation.");
  }


  std::vector<Eigen::VectorXd> u_ref_traj = computeReferenceThrust(reference_trajectory);
  ...
  std::vector<Eigen::VectorXd> x_seq_out;
  std::vector<Eigen::VectorXd> u_seq_out;
  ROS_WARN("x0 = constructStateVe");
  iLQR(x0, u_seq, horizon, dt, x_ref_traj, u_ref_traj, Q_, R_, Qf, x_seq_out, u_seq_out);

  // 8. 取优化后序列的第一步控制作为输出，并进行饱和处理
  Eigen::VectorXd u = u_seq_out[0];
  // saturateInput(u);
  command->timestamp = call_time;
  command->collective_thrust = u(0);
  command->bodyrates = u.segment(1, 3);
}
