...
class LQRController
{
  ...
  private:
    ...
    Eigen::VectorXd computeStateDot(const Eigen::VectorXd &x, const Eigen::VectorXd &u);
    
    VectorXd constructStateVector(
        const quadrotor_common::QuadStateEstimate& state);

    std::vector<Eigen::VectorXd> computeReferenceThrust(
          const quadrotor_common::Trajectory& ref);

    std::vector<Eigen::VectorXd> constructReferenceVector(const quadrotor_common::Trajectory& ref);
};
