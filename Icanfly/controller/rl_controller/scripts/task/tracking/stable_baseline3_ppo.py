...
# from commons.buffers import RolloutBuffer
# from commons.on_policy_algorithm import OnPolicyAlgorithm
# from commons.policies import ActorCriticCnnPolicy, ActorCriticPolicy, BasePolicy, MultiInputActorCriticPolicy
# from commons.type_aliases import GymEnv, MaybeCallback, Schedule
# from commons.utils import explained_variance, get_schedule_fn
...
class EWC:
    def __init__(self, policy: nn.Module, dataset: list):
        self.policy = policy
        self.dataset = dataset
        self.params = {n: p for n, p in self.policy.named_parameters() if p.requires_grad}
        self._means = {}
        self._precision_matrices = self._diag_fisher()
        for n, p in deepcopy(self.params).items():
            self._means[n] = variable(p.data.clone())
    ...
    def _diag_fisher(self):
        ...
        for input in self.dataset:
            ...
            dist = self.policy.get_distribution(input.unsqueeze(0))
            # 计算动作的均值和标准差
            # action = dist.sample()
...
class PPO(OnPolicyAlgorithm):
    ...
    def set_ewc_from_observations(self, observations):
        ...
        self.set_ewc(ewc, self.ewc_lambda)

    def set_ewc(self, ewc_object, importance: float):
        self.ewc = ewc_object
        self.ewc_lambda = importance
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                # 添加 L2 正则项
                if self.l2_lambda > 0:
                    l2_loss = sum((p ** 2).sum() for p in self.policy.parameters())
