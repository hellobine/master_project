...
quadrotor_common::ControlCommand MpcController<T>::run(
  ...
  // ROS_INFO("marker_pub_marker_pub_marker_pub_marker_pub_");
...
// template <typename T>
// void MpcController<T>::publishTrajectoryMarkers(const quadrotor_common::Trajectory& reference_trajectory) {
...
bool MpcController<T>::setReference(
  ...
  if (reference_trajectory.points.size() == 1) {
    ...
    //遍历 kSamples + 1 次，表示在每个采样点上计算参考状态。
